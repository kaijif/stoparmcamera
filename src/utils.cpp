...
char ST_SSID[32]  = "SJNAN"; //Default router ssid
char ST_Pass[MAX_PWD_LEN] = "221138221138"; //Default router passd
...
static void pingTimeout(esp_ping_handle_t hdl, void *args) {
  if (strlen(ST_SSID)) {
    ...
    if (wStat != WL_NO_SSID_AVAIL && wStat != WL_NO_SHIELD) {
      LOG_WRN("Failed to ping gateway,");
      // startWifi(false);
    }
  }
  doAppPing();
...
void dateFormat(char* inBuff, size_t inBuffLen, bool isFolder) {
  ...
  if (isFolder) strcpy(inBuff, "/videos");
  else {
    strftime(inBuff, inBuffLen, "/videos/%Y%m%d_%H%M%S", localtime(&currEpoch));
  }
}
